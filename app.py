...
def get_gender_test_data():
    test_data = pd.read_csv(r"gender_50devices.csv")
    return test_data

def get_age_test_data():
    test_data = pd.read_csv(r"age_50devices.csv")
    return test_data
...
def age_prediction():
    ...
    age_model = pickle.load(open('agePrediction.pickle', 'rb'))
    age_predict = age_model.predict(test_data.values)
    test_data["predicted_age"]= age_predict

    campain4=test_data[test_data["predicted_age"]<25]["device_id"][:15].to_list()
    campain5=test_data[test_data["predicted_age"].between(24,32)]["device_id"][15:35].to_list()
    campain6=test_data[test_data["predicted_age"]>32]["device_id"][:10].to_list()
...
def gender_prediction():
    ...
    gender_model = pickle.load(open("gender_prediction_model_events.pkl", 'rb'))
    x_test = test_data
